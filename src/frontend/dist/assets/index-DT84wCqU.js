...
const ZONE_SHRINK_INTERVAL = 3e4;
const ZONE_SHRINK_AMOUNT = 200;
const ZONE_DAMAGE_PER_SEC = 2;
const ENEMY_SPAWN_INTERVAL = 2e4;
...
function generatePickups() {
  const pickups = [];
  let id = 1e3;
  const kinds = ["pistol", "rifle", "shotgun"];
...
function spawnEnemy(id, obstacles) {
  let x;
  let y;
...
function createInitialState() {
  ...
  return {
    ...
    zoneX: MAP_W / 2,
    zoneY: MAP_H / 2,
    zoneShrinkTimer: ZONE_SHRINK_INTERVAL / 1e3,
    zoneWarning: 0,
    enemySpawnTimer: ENEMY_SPAWN_INTERVAL / 1e3,
    nextId: 2e3,
...
function drawGrass(ctx, cameraX, cameraY, w, h) {
...
function drawObstacles(ctx, obstacles, cameraX, cameraY, w, h) {
  for (const obs of obstacles) {
...
function drawTrees(ctx, trees, cameraX, cameraY, w, h) {
  for (const tree of trees) {
    const sx = tree.x - cameraX;
    const sy = tree.y - cameraY;
    if (sx + tree.r < -10 || sx - tree.r > w + 10 || sy + tree.r < -10 || sy - tree.r > h + 10)
      continue;
...
function drawPickups(ctx, pickups, cameraX, cameraY) {
  for (const p of pickups) {
    ...
    const label = p.kind === "medkit" ? "+HP" : p.kind === "ammo" ? "AMM" : p.kind.substring(0, 3).toUpperCase();
    ctx.fillText(label, sx, sy + size / 2 + 10);
  }
}
function drawPlayer(ctx, player, cameraX, cameraY) {
...
function drawEnemies(ctx, enemies, cameraX, cameraY) {
...
function drawBullets(ctx, bullets, cameraX, cameraY) {
...
function drawFloatTexts(ctx, floatTexts, cameraX, cameraY) {
...
function drawHUD(ctx, gs, w, h, elapsed) {
  const { player } = gs;
...
function drawBar(ctx, x, y, w, h, ratio, bgColor, fillColor) {
...
function drawMinimap(ctx, gs, canvasW, canvasH) {
  const MM_SIZE = 160;
...
function drawJoystick(ctx, touch) {
...
function updateGame(gs, dt, canvasW, canvasH) {
  ...
  if (shouldFire && player.fireCooldown <= 0) {
    ...
    if (ammoData.mag > 0) {
      ...
      for (let p = 0; p < wepDef.pellets; p++) {
        const spreadAngle = player.angle + (Math.random() - 0.5) * wepDef.spread;
        gs.bullets.push({
          id: gs.nextId++,
  ...
  for (let bi = 0; bi < gs.bullets.length; bi++) {
    ...
    if (!remove) {
      if (b.fromPlayer) {
        ...
        if (dist(b.x, b.y, player.x, player.y) < PLAYER_RADIUS + BULLET_RADIUS) {
          let dmg = b.damage;
          if (player.armor > 0) {
  ...
  if (gs.enemySpawnTimer <= 0) {
    gs.enemySpawnTimer = ENEMY_SPAWN_INTERVAL / 1e3;
  ...
  if (gs.zoneShrinkTimer <= 0) {
    gs.zoneShrinkTimer = ZONE_SHRINK_INTERVAL / 1e3;
    gs.zoneRadius = Math.max(
      ZONE_MIN_RADIUS,
      gs.zoneRadius - ZONE_SHRINK_AMOUNT
    );
  }
  gs.zoneWarning = gs.zoneShrinkTimer <= 5 && gs.zoneRadius > ZONE_MIN_RADIUS ? gs.zoneShrinkTimer : 0;
...
function Game() {
  ...
  const [finalStats, setFinalStats] = reactExports.useState({ kills: 0, time: 0 });
  const screenRef = reactExports.useRef("start");
  ...
  reactExports.useEffect(() => {
    ...
    const onTouchMove = (e) => {
      ...
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (t.identifier === gs.touch.joystickId) {
          ...
          if (dl > maxDist) {
            gs.touch.joystickCurrX = gs.touch.joystickBaseX + ddx / dl * maxDist;
            gs.touch.joystickCurrY = gs.touch.joystickBaseY + ddy / dl * maxDist;
          } else {
            gs.touch.joystickCurrX = t.clientX;
            gs.touch.joystickCurrY = t.clientY;
          }
    ...
    const loop = (time) => {
      ...
      const rawDt = (time - lastTimeRef.current) / 1e3;
      ...
      if (screenRef.current === "playing") {
        ...
        if (!gs.player.alive) {
          const elapsed2 = (performance.now() - gs.startTime) / 1e3;
          setFinalStats({ kills: gs.player.kills, time: elapsed2 });
      ...
      const elapsed = (performance.now() - gs.startTime) / 1e3;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        screen === "start" && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            style: {
              position: "absolute",
              ...
              background: "linear-gradient(160deg, #0B0F12 0%, #111820 50%, #0d1008 100%)",
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "2D TOP-DOWN SHOOTER GAME"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "h1",
                {
                  style: {
                    ...
                    lineHeight: 1,
                    color: "#E8E0CC",
                    textShadow: "0 0 60px rgba(255,90,31,0.3), 0 4px 0 #0B0F12, 0 8px 0 rgba(0,0,0,0.5)",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  ...
                  style: {
                    ...
                    textTransform: "uppercase",
                    background: "linear-gradient(135deg, #FF5A1F 0%, #FF8C42 50%, #FF5A1F 100%)",
                    color: "#0B0F12",
                    border: "none",
                    borderRadius: "4px",
                    cursor: "pointer",
                    boxShadow: "0 0 30px rgba(255,90,31,0.5), 0 0 60px rgba(255,90,31,0.2), inset 0 1px 0 rgba(255,255,255,0.2)",
                    transition: "transform 0.1s, box-shadow 0.1s"
                  },
                  onMouseEnter: (e) => {
                    e.target.style.transform = "scale(1.05)";
                    e.target.style.boxShadow = "0 0 40px rgba(255,90,31,0.7), 0 0 80px rgba(255,90,31,0.3), inset 0 1px 0 rgba(255,255,255,0.2)";
                  },
                  onMouseLeave: (e) => {
                    e.target.style.transform = "scale(1)";
                    e.target.style.boxShadow = "0 0 30px rgba(255,90,31,0.5), 0 0 60px rgba(255,90,31,0.2), inset 0 1px 0 rgba(255,255,255,0.2)";
                  },
                  children: "▶ PLAY NOW"
                }
              ),
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                style: {
                  ...
                  textAlign: "center",
                  boxShadow: "0 0 60px rgba(255,90,31,0.15), 0 0 120px rgba(0,0,0,0.8)",
                  maxWidth: "480px",
                  width: "90%"
                },
                children: [
...
function drawStartScreen(ctx, w, h, _time) {
  ctx.clearRect(0, 0, w, h);
